...
bool RemindShader::init()
{
	...
	for (auto i = 0; i < REMIND_RENDER_COUNT; i++)
	{
		m_ptRenderPos[i] = Vec2::ZERO;
	}
...
//    }
	...
	if (m_nRenderCount == 0)
    {
		...
		for (auto i = 0; i < REMIND_RENDER_COUNT; i++)
		{
			if (m_ptRenderPos[i] == Vec2::ZERO)
			{
				m_ptRenderPos[i] = m_ptCurRenderPos;
			}
		}
...
void RemindShader::Render()
{
	...
	{    
		...
		//log("delta %.2f ~~~~~ %.2f", delta.x, delta.y);
		delta = Vec2::ZERO;
